...
ErrorCode sendBroadcast(const SendInfo* const info, OnResponse onResponse)
{
    ...
    setupBroadcastAddress(&broadcast_addr, info->socket.port, info->socket.ip);
    ...
    while (true) 
    {
        ...
        if (sendSocketMessage(sock, info->msg, &broadcast_addr) == FAIL)
        {
            goto fail;
        }
        ...
        if (receiveSocketResponse(sock, buffer, MIDDLE_BUFFER_SIZE, &recv_addr, &addr_len) == FAIL) 
        {
        ...
        {
            onResponse(buffer, getIpString(&recv_addr), getPort(&recv_addr));
            break;
        }
    }
